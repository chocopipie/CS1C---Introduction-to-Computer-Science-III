...
int main()
{
	...
	cout << "*         CS1C T Th 12:30 - 2:50     * " << endl;
	cout << "************************************** " << endl;
	cout << endl;
...
or derived from some common ancestor type
Containter "owns" the elements (control creation and destruction of objs)
- An STL algorithm defines a collection of functions esp designed to be used on
ranges of elements. They provide means for various operations for the contents of
the containers.
- An STL iterator is an obj (like a pointer) used to point to an element inside the
container. We can use iterators to move through the contents of the container

// Q#2
- By dereferencing the iterator, we can get the pointed to object. Thus, it allows
the simplicity of algorithm with less inputted numbers such as size and make the 
algorithm generic
