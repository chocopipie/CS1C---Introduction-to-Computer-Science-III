...
{
public:
    Less_than(int value) : v(value) {}  // initialize value to v when the class is instantiated
...
struct Shape
{
    Shape(int id) : id{id} { }
    virtual ~Shape() { }
    virtual int calc_perimeter() const { return -1; }
...
public:
    Range_low_to_high(Val lowValue, Val highValue) : low{lowValue}, high{highValue} {}
...
// Q#1
// 1. InputIterator first, last - an iterator that refers to some obj (first and after last)
//                                 in the input sequence
//                          An iterator is past-the-end if it points beyond the last element
// 2. EqualityComparible val - the type can be compared for equality  using operator ==

// Q#5
// copy_if() function performs copy operation according to the given condition
// it takes 4 arguments:
//  1. In first - the input iterator of the first element of the container (input sequence)
//  2. In last - the input iterator one past the end of the container (input sequence)
//  3. Out result - the output iterator pointing to the element where result is written into
//  4. Pred pred - the predicate to test the element in the given range, to see if
//                  the element satisfies the condition or not
